 ...
 *
 * Not verified: how the site orders results by default and whether a query parameter changes it, so
 * `sortByDateParam` stays unset. The provider sorts what it read by `advertised_date` itself, which is
 * right whatever the site's order is - but only across the pages it reads (see `MAX_PAGES`).
 */
