...
import { getCurrencyForProvider } from './services/providers/providerCurrency.js';
...
class FredyPipelineExecutioner {
  ...
  async _notify(newListings) {
    ...
    const currency = await getCurrencyForProvider(this._providerId);
    const formattedListings = newListings.map((listing) => formatListing(listing, language, currency));
