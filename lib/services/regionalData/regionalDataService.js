 ...
 * (kommuneskat + grundskyld) and school markers (grade average + inclusion percentage).
 *
 ...
 *
 * A school's coordinates are not expected to be part of the STIL cube itself (see
 * `schoolClient.js#normalizeSchoolRow`) - once the exact response shape is confirmed with a real
 * key, a school that only carries an address/postnummer can be placed on the map by running it
 * through `lib/services/geocoding/geoCodingService.js#geocodeAddress`, the same helper listings use.
 * That wiring is deliberately left out here rather than guessed at against an unverified API shape.
 */

const KOMMUNE_CACHE_KEY = 'kommuner';
const TAX_CACHE_KEY = 'kommuneTaxRates';
const SCHOOL_CACHE_KEY = 'schoolStats';
  ...
  const rates =
    kommuner.length === 0
      ...
      : ((await cached(
          ...
          () => fetchKommuneTaxRates(kommuner.map((k) => k.code)),
        )) ?? new Map());
...
export async function getSchoolLayer() {
  ...
  return {
    available: true,
    // Coordinate-less rows are kept out of a map layer that can only place a marker somewhere -
    // see the module doc comment above for why they are not geocoded here.
    schools: (schools ?? []).filter((school) => school.lat != null && school.lng != null),
    attribution: [UDDANNELSESSTATISTIK_ATTRIBUTION],
  };
}
