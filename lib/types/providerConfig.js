 ...
 * @property {string[]} [searchHosts] Host(s) a pasted search URL is actually expected to live on, when that differs from `baseUrl`'s host - a provider reached through an API subdomain the job form never links to (see boligsiden.js). Read by `ui/src/services/jobs/providerUrl.js`; absent means `baseUrl`'s host is the only one accepted, which is every other provider.
 */
