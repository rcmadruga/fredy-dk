 ...
 * @property {string} baseUrl The portal's root, used to build absolute links out of relative ones and as the "open in new tab" link in the job form.
 ...
 * @property {string} [currency] ISO 4217 code the provider's prices are in. Optional, defaults to `EUR`, which is what every provider outside Denmark lists in. Read by the notification formatter (`lib/utils/formatListing.js`) and by the UI, which prints each listing's price in its own provider's currency rather than assuming euros.
 ...
 * @property {string[]} [hosts] Every host a pasted search url is accepted on, *in addition to* `baseUrl`'s own host - for a portal that serves several countries as several domains (immowelt on `.de`/`.at`, idealista on `.com`/`.it`/`.pt`). Read by `ui/src/services/jobs/providerUrl.js`; absent means `baseUrl`'s host is the only one accepted.
 * @property {string[]} [searchHosts] Host(s) a pasted search URL is actually expected to live on, when `baseUrl`'s own host is never a valid one to paste from - a provider reached through an API subdomain the job form never links to (see boligsiden.js). Unlike `hosts`, this *replaces* `baseUrl` rather than adding to it and takes precedence when both are declared. Read by `ui/src/services/jobs/providerUrl.js`.
 */
