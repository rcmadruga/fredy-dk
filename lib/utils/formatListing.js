...
import { DEFAULT_CURRENCY, currencySymbol } from '../services/providers/currencies.js';
 ...
 * @param {string} [currency='EUR'] ISO 4217 code of the provider's prices, from its `metaInformation`.
 * @returns {FormattedListing} A copy of the listing with formatted strings for price, size, and rooms.
 */
export const formatListing = (listing, language = 'en', currency = DEFAULT_CURRENCY) => {
  const rooms = ROOMS_LABEL[language] ?? ROOMS_LABEL.en;
  return {
    ...listing,
    price: listing.price != null ? `${listing.price} ${currencySymbol(currency)}` : null,
 ...
 * @param {string} [currency='EUR'] ISO 4217 code of the provider's prices.
 * @returns {FormattedPriceChange}
 */
export const formatPriceChange = (change, language = 'en', currency = DEFAULT_CURRENCY) => {
  const labels = PRICE_CHANGE_LABEL[language] ?? PRICE_CHANGE_LABEL.en;
  const percent = Math.abs(change.changePercent).toFixed(1);
  return {
    ...formatListing(change.listing, language, currency),
    oldPrice: `${change.oldPrice} ${currencySymbol(currency)}`,
    newPrice: `${change.newPrice} ${currencySymbol(currency)}`,
